...
const Premium = () => {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        const clientResponse = await fetch('http://localhost:3000/api/token/conta/cliente', {
          method: 'GET',
          headers: {
        ...
        if (clientResponse.ok) {
          ...
          const response = await fetch(`http://localhost:3000/api/cliente/acesso/${clienteId}`, {
            method: 'PUT',
            headers: {
